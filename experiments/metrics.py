...
def bootstrap_ci(data: np.ndarray, statistic=np.mean, n_resamples: int = 10000, ci: float = 0.95) -> Tuple[float, float]:
    """Bootstrap confidence interval for a statistic."""
    n = len(data)
    bootstraps = np.array([statistic(np.random.choice(data, size=n, replace=True)) for _ in range(n_resamples)])
    alpha = (1.0 - ci) / 2.0
...
def permutation_test(control: np.ndarray, treatment: np.ndarray, n_permutations: int = 10000) -> float:
