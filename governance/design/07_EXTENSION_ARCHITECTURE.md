...
**Why no redesign:** §2's Scope definition includes "population, environment, conditions, versions, and time interval," so a Registry entry can be scoped to a repository. Cross-repository references use DG-7 custody manifests, which already handle content-addressed external artifacts.
...
**Mechanism:** delegation chains under DG-3, each recording subject, Scope, recipient, permitted transitions, and limits (§4 ¶4), with sub-delegation containment enforced by `DA-37`.
...
**Mechanism:** a Federation Domain standard owning `external_partner`, `shared_protocol`, and `external_attestation`, plus DG-7 custody arrangements for artifacts held by partners.
...
### 2.7 AI reviewers
**Mechanism:** AI review is a first-class **input** at stage V4 (adversarial review) and may implement any check in DG-11's register.
...
### 2.8 Human reviewers
**Mechanism:** authority assignments and conflict-of-interest declarations under DG-3; attestations under DG-4.
...
**Constraint:** §2's independence test is per-review, not per-person. A reviewer independent for one change may be conflicted for the next. DG-4 must therefore record independence *per attestation*, which it does.
...
|---|---|---|---|
| DX-1 | Object type register | `ontology/TYPE_REGISTER.yaml` | adding a type + naming its owning standard |
| DX-2 | Relationship kinds | `ontology/RELATIONSHIPS.yaml` | adding an edge kind with endpoint type constraints |
| DX-3 | Identifier namespaces | `ontology/IDENTIFIERS.yaml` | allocating a namespace; never reusing a retired id |
| DX-4 | Domain standards | `domain_standards/{class}/` | one activation dossier per standard |
| DX-5 | Domain classes | `domain_standards/` subdirectories | adding a class directory; `engineering/` already reserved and empty |
| DX-6 | Check register | `automation/CHECK_REGISTER.yaml` | adding a check with FP/FN disclosure |
| DX-7 | Manual procedures | the `MRP-xx` register | adding a procedure; required whenever a new non-decidable MUST appears |
| DX-8 | Authority assignments | `governance/authority/` | assignment + succession |
| DX-9 | Delegations | `governance/delegations/` | five-element record, contained in its parent |
| DX-10 | Digest algorithms | DG-7's algorithm register | registering a new algorithm; deprecating an old one without invalidating history |
| DX-11 | Custody backends | `custody/` manifests | new backend with identity, provenance, recovery procedure |
| DX-12 | Record stores | repository root | new top-level store + `_BOUNDARY.yaml` + owning standard |
| DX-13 | Scope dimensions | jurisdiction declarations | adding a controlled dimension to sharpen `DA-04` |
| DX-14 | External anchors | DG-11 + DG-7 | signing, timestamping, mirroring — additive, no governance change |
...
**Identifiers.** Never reused, never renumbered, never repurposed. An identifier scheme that survives is one where a citation written in year one still resolves in year twenty. Retired ids stay retired (DX-3).

**Digests.** At least one registered algorithm will be considered broken within twenty years. DG-7's versioned algorithm register and linked re-digest procedure handle this without invalidating history — provided re-digesting is recorded as a new manifest linked to the prior one, never as an edit.
