...
**Detection.** `A-23`, `A-38`, and direct inspection of `GOVERNANCE_REGISTRY.yaml` (`constitutional_steward: []`, both attestations `null`).
...
**Detection.** `A-07`, `A-30`; the M-1.8 legacy inventory.
...
**Consequence.** The confirmatory/exploratory distinction — the load-bearing guarantee of the entire scientific stack — rests on a single actor's honesty. `A-20` verifies only that no *recorded* execution precedes registration.
**Detection.** `A-20`, registered **Advisory** by construction. Reporting it Active would misrepresent a heuristic as a proof.
...
**Detection.** `A-29`, `A-51`, `A-61`; every check asserting a non-empty target set.
**Mitigation.** M-1.5 CI repair; prohibit exit-code suppression in any conformance-relevant check; make Advisory a registration status rather than a runtime trick; G-5 claims enumerate criteria actually assessed.
...
**Detection.** `A-10`, `A-27`; the M-2 reconciliation.
...
**Mechanism.** §7 ¶4 forbids deleting, hiding, or relabelling Negative Results. But a Result never recorded is undetectable: `A-60` can only compare records that exist.
**Consequence.** Selective recording produces a literature-shaped bias inside a repository designed to prevent it.
**Detection.** `A-60` (Advisory, statistical); `MRP-17` reconciles execution evidence against recorded Results.
...
**Mechanism.** §2 makes the Registry determinative of what is Active; §4's precedence order omits it (defect D-2, finding F-6). A Registry/standard conflict is formally unresolvable: the standard's authority depends on the entry, and the entry's correctness depends on the standard.
...
**Mitigation.** Amend §4 to state that the Registry determines *whether* an artifact has authority, never *what* it requires (H-04); include this in the adoption change while amendment is cheap. Append-only `registry/transitions/` permits reconstruction. `A-01`/`A-03` detect drift.
...
**Mitigation.** `04_DOMAIN_STANDARD_INTERFACE.md` §6: interface MAJOR changes require a named migration plan covering every Active standard in the same change. `A-02`/`A-03` detect the state, but detection after the fact does not undo the void transitions.
...
**Mitigation.** Every check asserts a non-empty target set and reports `NOT_VERIFIED` otherwise; self-test fixtures with known-violating inputs, so a check that passes its own negative fixtures is Broken by definition; `A-50` register completeness.
...
**Mitigation.** G-7's versioned algorithm register; deprecation without invalidating history; re-digesting recorded as a new linked manifest, never an edit; `A-43`, `A-44`.
**Residual.** Low.

### R-22 · Forged records in a single-writer repository — High
**Mechanism.** Authority assignments, attestations, and transition records are plain files. Absent cryptographic signing, anyone with write access can create a record purporting to be another party's approval, and `A-05`/`A-38`/`A-53` verify only internal consistency.
...
**Mitigation.** Platform-level branch protection on `results/`, `evidence/`, `observations/`, `decisions/`, `unknowns/`, `constitution/attestations/`, `registry/transitions/`; an off-repository mirror; `A-14`, `A-15`, `A-45`.
**Residual.** Low with protection and a mirror; otherwise undetectable.

### R-23 · Requirement coverage decay — Medium
**Mechanism.** `A-39` maps every MUST to a check, an `MRP`, or a testable non-applicability statement. The Constitution has no clause identifiers (finding F-19), so coverage against it must be hand-curated.
...
**Mitigation.** The **Peer Reference Rule** — never restate a peer's requirement — is the primary structural defence, because most same-level conflicts arise from duplicated text drifting apart. Plus `A-28` (Advisory), `MRP-04`, G-10 conflict records.
**Residual.** Medium.
...
**Mitigation.** Every standard cites its constitutional clause and states its failure modes; every architecture decision records rationale and rejected alternatives; `A-34` boundary declarations are executable, not prose. A successor should be able to reconstruct *why*, not only *what*.
...
**Mitigation.** G-9's jurisdiction owns exactly one object type; `A-48` verifies no containment record authorizes a non-containment transition; every containment action remains subject to review by §12 ¶2's own terms.
**Residual.** Low with the narrow jurisdiction; High if G-9 is drafted expansively.
...
**Mitigation.** `A-38` requires an identified human and a competence statement; V4 records AI output as findings; V6 attestations state which findings the human examined. Escalated as H-07 so the boundary is affirmed deliberately rather than assumed.
...
**Detection.** `A-25` scanning full history, not only HEAD.
...
| R-10 | Registry unranked / single point | High | realized (D-2) |
