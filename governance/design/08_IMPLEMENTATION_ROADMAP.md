...
3. **The dependency graph is the activation order.** O-1 gates every type-owning standard; DG-3 gates every authorized transition; DG-4 gates every attestation and therefore every activation; DG-13 gates every scientific state change.
...
- H-03 — whether to include amendment **DAM-8** (General Delegation Clause, defect DD-1) in the adoption change. Doing it at adoption is far cheaper than amending an Active Constitution later, and DG-2, DG-4, DG-5, and DG-12 all depend on it.
- H-04 — whether to include the Registry precedence clause (defect DD-2, finding F-6).
- H-12 — whether to add clause identifiers (finding F-19), on which `DA-39` coverage depends.
...
| # | Deliverable | Why now |
|---|---|---|
| M-1.1 | Draft O-1, DG-1, DG-2, DG-3, DG-4, DG-13 to the Domain Standard Interface | These six gate everything; drafting is the long-lead item |
| M-1.2 | Machine-readable jurisdiction declarations for all 24 standards | Makes `DA-04` possible; forces the disjointness discipline early, when it is cheap to fix |
| M-1.3 | Registry JSON Schema | `DA-01`, `DA-03` |
| M-1.4 | Build checks DA-01…DA-09, DA-22, DA-29, DA-36, DA-38, DA-39, DA-50, DA-51 as **Advisory** | Governance-header and Registry checks are the cheapest high-value automation and need no authority to run |
...
| M-1.7 | External anchors: commit signing, branch protection on record paths, third-party timestamping | Closes the four structurally-undetectable classes in `05_AUTOMATION_ARCHITECTURE.md` §5. **Timestamping must precede any confirmatory work**, or `DA-20` remains Advisory forever |
...
**O-1, DG-1, DG-2, DG-3, DG-4, DG-13.**

**Exit criteria:** `DA-04` reports no overlap; every registered type has exactly one owner; every declared transition resolves to a living authority (`DA-05`); every MUST in the six standards maps to a check or an `MRP-xx` (`DA-39`).
...
Activate **S-1, S-3, S-4, S-8, DG-7, DG-12.**

This is the minimum set that permits a scientific execution to produce a Result: a Registered protocol (S-1), Observations (S-4), Results (S-8), Unknowns (S-3), custody (DG-7), and an Active architecture record (DG-12, discharging the §10 nonconformance N-1).

Promote checks DA-12…DA-17, DA-20, DA-21, DA-31…DA-34, DA-43…DA-45, DA-58…DA-60 from Advisory to Active as their requirements come under Active standards.
...
Activate **S-2, S-5, S-6, S-9.** Claims, Evidence admission, Interpretations, Questions. Promote DA-17…DA-19, DA-54…DA-56, DA-62.
...
Every artifact made Legacy at adoption gets an explicit disposition under DG-2. Three outcomes only:
...
**Do not defer this.** While Legacy artifacts sit in live paths asserting authority, `DA-07` and `DA-30` produce standing findings, and — more importantly — contributors will continue to follow them. The gap between "has no authority" and "is not being followed" is where governance systems actually fail.
...
Activate **DG-5, DG-6, DG-8, DG-11.** Conformance claims, audit, change and release, the check register itself. Promote the remaining checks; register every `MRP-xx`.

**Exit:** `DA-39` reports full requirement coverage across all Active standards; the first conformance claim and first constitutionally valid audit are published.

---

### M-8 · Maturity *(gate: M-7)*

Activate **S-7, S-10, DG-9, DG-10.** Hypotheses, Principles, containment, conflict records. These are last because Hypotheses and Principles depend on the full inference chain, and containment and conflict machinery is only meaningful once there is something to contain or conflict.
...
M-2 ontology reconciliation
   ↓
M-3 foundation      O-1 DG-1 DG-2 DG-3 DG-4 DG-13
   ↓
M-4 execution       S-1 S-3 S-4 S-8 DG-7 DG-12   ──► first valid Result
   ↓
M-5 inference       S-2 S-5 S-6 S-9            ──► first valid Claim state change
   ↓
M-7 assurance       DG-5 DG-6 DG-8 DG-11
   ↓
M-8 maturity        S-7 S-10 DG-9 DG-10
...
**Do not do:** activate a standard whose owned types O-1 has not registered; activate DG-2 while DD-1 is unresolved (H-03); begin confirmatory work before M-4; treat any current `artifacts/exp_*` output as Evidence.
