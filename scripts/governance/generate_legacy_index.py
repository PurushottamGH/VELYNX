...
TIER1 = {
    "theory/PROGRAM_D_CONSTITUTION.md": (
        "archive/legacy_normative/",
        "titled as a constitution; asserted program-wide authority",
    ),
    "PROGRAM_D_CANONICAL.md": (
        "archive/legacy_normative/",
        "asserted canonical status for Program D content",
    ),
    "PROGRAM_D_MASTER_ROADMAP.md": (
        "archive/legacy_normative/",
        "asserted frozen scope and sequencing",
    ),
    "PROGRAM_D_RESEARCH_STATE_v0.1.md": (
        "archive/legacy_normative/",
        "asserted current research state as settled",
    ),
    "RESEARCH_PROTOCOL.md": (
        "archive/legacy_normative/",
        "read as the governing research procedure",
    ),
    "audits/CONSTITUTION_COMPLIANCE.md": (
        "archive/legacy_normative/",
        "asserted compliance against a superseded constitution",
    ),
    "p1/specifications/milestone-1-gpt56.md": (
        "(stays in place)",
        "imposes PI-003/PI-004/PI-005; imported by p1/tooling, so moving it breaks working code for no compliance gain",
    ),
}
...
TIER3_RULES = [
    (
        "archive/",
        "Inherited Programs A/B/C material and legacy documents. Retained for history; nothing depends on it.",
    ),
    (
        "docs/",
        "Documentation, sprint planning, migrations, and model-authored audits. Descriptive; imposes nothing.",
    ),
    (
        "audits/",
        "Audit and checklist reports. Under section 3 paragraph 5 an audit reports observations and MUST NOT create the requirement it audits.",
    ),
    (
        "evidence/",
        "Verification artifacts, certifications, traces, ledgers. Certification language here confers no authority.",
    ),
    (
        "artifacts/",
        "Nineteen exp_* execution outputs. Permanently exploratory working output: no Registered protocol preceded them (sections 2, 5 paragraph 3, 13 paragraph 4).",
    ),
    (
        "research/",
        "Inherited research scaffolding and preregistration-shaped documents. Not Registered protocols.",
    ),
    (
        "reviews/",
        "Review-shaped records. Section 4 paragraph 4 bars automation from supplying independent review; these do not satisfy section 13 paragraph 2.",
    ),
    (
        ".agents/",
        "Agent and skill configuration. Several define reviewer or auditor roles for models; none can supply human authority or independent review (section 4 paragraph 4).",
    ),
    (".opencode/", "Agent configuration, as above."),
    (".commandcode/", "Agent configuration, as above."),
    (
        "backend/constitution/",
        "Model-behaviour prompt files. The directory name collides with the Constitution and carries no governance meaning (section 11 paragraph 3).",
    ),
]
...
def main() -> int:
    ...
    a("")
    a(
        'Section 14: "At adoption, every existing Normative artifact not activated in the '
    ...
    a("")
    a(
        "Classification follows automatically from the Registry. This index records it so a "
    ...
    a("")
    a(
        "`GOVERNANCE_REGISTRY.yaml` is the only answer. At adoption the Active set is "
    ...
    a("")
    a(
        "`Current path` is where the banner is verified. Until change D moves the file it "
    ...
    a("")
    a(
        "The moving commit is deliberately not recorded here. It is the revision that "
    ...
    a("")
    a(
        f"{len(tier2)} artifacts in `theory/`, `experiments/`, and `p1/` that either assert "
    ...
    if prereg:
        ...
        a("")
        a(
            "These require the second banner paragraph in "
    ...
    a("")
    a(
        "**Method.** Tier 1 is a hand-curated list of artifacts that describe themselves as "
    ...
    a("")
    a(
        "**Known false negative.** A Normative artifact that imposes requirements without "
    ...
    a("")
    a(
        "**Not mechanically decidable.** Whether a document *reads* as authoritative is manual "
    ...
    a("")
    a(
        "A Legacy artifact carries force again only by satisfying section 4: complete "
    ...
    print(
        f"wrote {rel(OUT)}: tier1={len(TIER1)} tier2={len(tier2)} "
