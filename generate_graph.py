...
LAWS = [
    ("L1","HebbianLearning","Fire together, wire together — coactivation count increments, edge at count>=2, asymptotic weight updates"),
    ("L2","FreeEnergyProxy","Cognitive energy E = λ·H + μ·S + ν·A (λ=1.0, μ=2.0, ν=0.5) — stand-in for variational free energy"),
    ("L3","DirichletMarkovModel","P(s|c) = (n(c,s)+α)/(N(c)+α·|support|) — additive smoothing with symmetric Dirichlet prior"),
    ("L4","ShannonSurprise","I(s) = -log₂ P(s|c) — surprisal in bits, the universal cognitive currency"),
    ("L5","PrecisionWeightedAttention","a(s) = 1−P(s|c) ∈ [0,1]; attention = precision of the prediction residual"),
    ("L6","KLDivergence","KL(P‖Q) = Σ P(s) log₂(P(s)/Q(s)) — confidence delta per tick"),
    ("L7","InertiaLaw","effective_rate = base_rate × (1−stability); mature rules barely move at an anomaly"),
    ("L8","ExceptionQuarantine","Contradictions filed as BeliefExceptions inside parent belief, never overwrite the rule"),
    ("L9","FractureMechanics","Rule fractures when contradiction_count / support_count ≥ 0.85; dominant exception promoted"),
    ("L10","StabilityCurve","stability(n) = ln(1+n) / (ln(1+n)+ln(1+4)) — logarithmic confidence-in-history measure"),
    ("L11","SynapticPruning","asymptotic_weight ×= (1.0−decay_rate); edges below prune_threshold deleted"),
    ...
    ("L14","NPMIClustering","Normalised Pointwise Mutual Information (−1,1) for co-occurrence matrices"),
    ("L15","SpectralClustering","Eigengap heuristic + K-Means++ on Laplacian for concept birth"),
    ("L16","BayesianEdgeEvaluation","Living edges tracked with Beta posterior: quality, reinforced_count, challenged_count → confidence state"),
    ("L17","ThermodynamicState","Scalar parameter controlling strictness vs. exploration in reasoning, from conflict state"),
    ("L18","ResonancePropagation","Multi-hop semantic wave propagation: compound resonance = 1−(1−curr)×(1−new_energy)"),
    ("L19","CognitiveProgressIndex","CPI = weighted composite of accuracy (0.4), decay (0.3), compression (0.2), resolution (0.1)"),
    ("L20","FreeEnergyMinimization","C8 merge policy: accept iff E_after ≤ E_before (ΔE ≥ 0)"),
    ("L21","ParetoDominance","C8 merge: reject only if all four vitals (H,S,A,E) worsen simultaneously"),
]

ASSUMPTIONS = [
    ("A1","SymbolAsAtom","Cognition operates on discrete symbols each with positive probability via Dirichlet smoothing"),
    ("A2","FlatMarkovSufficient","A single-level Markov chain captures enough predictive structure; hierarchy unnecessary"),
    ("A3","StabilitySolvesForgetting","Inertia Law + Exception Quarantine prevents catastrophic forgetting without Fisher info"),
    ("A4","SurpriseEqualsDissonance","Shannon surprisal captures all forms of cognitive dissonance (no valence/signed errors)"),
    ("A5","AttentionIsPrecisionResidual","Attention = 1−P(obs|context); no biased competition, spatial, or overt/covert attention"),
    ("A6","LocalRulesSufficient","Rate-based Hebbian coactivation suffices; no STDP, three-factor, or backprop needed"),
    ("A7","FreeEnergyProxyIsAdequate","E = λH+μS+νA is a sufficient stand-in for variational free energy"),
    ("A8","ConceptBirthWorksFromStats","Spectral clustering on NPMI-thresholded co-occurrence suffices for concept formation"),
    ("A9","SingleLevelPredictionWorks","Flat predict/collide/adapt loop is enough; no hierarchical predictive coding needed"),
    ("A10","HandAuthoredScaffoldingOk","Seeded curriculum and pre-authored ontology acceptable; emergence not required"),
    ("A11","CPIMeasuresLearning","Four-component CPI (accuracy, decay, compression, resolution) captures genuine becoming"),
    ("A12","ProbationaryTestingWorks","Probation over N deviation trials with coverage threshold validates latent abstractions"),
    ("A13","MetaphoricalNeuroscienceOk","Terms like resonance, thermodynamic state, sleep cycle need no literal biological mechanism"),
    ("A14","RateBasedHebbianSufficient","Rate-based coactivation counts capture Hebbian learning without spike-timing"),
    ("A15","EdgeWeightAsConfidence","Asymptotic weight + Bayesian state fully capture edge reliability"),
    ("A16","ConjugatePriorsTractable","Beta and Dirichlet conjugate priors keep Bayesian computations tractable"),
]

VARIABLES = [
    ("V1","SurpriseBits","prediction_error = -log₂ P(obs|context); the universal cognitive signal (bits)"),
    ...
    ("V5","FractureRatio","contradiction_count / support_count; approaches 1.0 when rule is candidate for revision"),
    ...
    ("V8","EffectiveLearningRate","base_rate × (1−stability); Inertia Law governs belief movement"),
    ("V9","H_Entropy","Conditional Shannon entropy of cluster-transition Markov chain (bits)"),
    ("V10","S_SurpriseSpatial","Euclidean distance between predicted centroid and observed vector"),
    ...
    ("V13","PredictionAccuracyTrend","EWMA of 1−normalized(surprise); ∈ [0,1]; CPI accuracy component"),
    ("V14","AverageSurpriseDecay","EWMA of recovery fraction after novelty spike; CPI decay component"),
    ("V15","ConceptCompressionRatio","experiences / consolidated concepts; ≥ 1; CPI compression component"),
    ("V16","ContradictionsResolved","Monotone counter of reified contradictions; CPI resolution component"),
    ("V17","ThermodynamicStateValue","Scalar controlling reasoning strictness vs. exploration (from conflict)"),
    ("V18","ResonanceScore","Multi-hop activation propagation value; compound resonance formula"),
    ("V19","EdgeAsymptoticWeight","Living edge strength after Hebbian reinforcement; decays over time"),
    ("V20","ProbationCoverage","hits / trials for a probationary concept; ≥ 0.60 to confirm"),
    ("V21","PromisedBits","Predictive bits a concept birth promises to save (H_before − H_after)"),
    ...
    ("V24","EnergyExhaustionThreshold","E ≥ 18.0 signals fragmentation and triggers concept birth"),
    ("V25","NPMIScore","Normalised Pointwise Mutual Information between features"),
]

EXPERIMENTS = [
    ("E1","C1_PredictiveUnderstanding","Engine holds a standing prediction; reality collides to produce surprise, driving attention/learning/concept formation"),
    ("E2","C2_ExplainPrediction","Every forecast carries a rationale and localized world model (rule + exceptions)"),
    ("E3","C3_PredictionFailureAnalysis","When surprise breaches threshold, engine authors causal_hypothesis positing missing intermediate cause"),
    ("E4","C5_CuriosityEngine","Curiosity selects inquiry maximizing EIG over structural hypotheses about concept-trait relations"),
    ("E5","C6_StableBeliefRevision","Multidimensional beliefs with Inertia Law, Exception Quarantine, and Fracture mechanics"),
    ("E6","C7_SensoriumMonitor","Continuous loop: world→SensorArray→VectorPredictionCore→FreeEnergy vitals→on EXHAUSTION mine quarantine"),
    ("E7","C8_DecisionPolicy","Staged memory restructuring: ReplayEngine sandbox-simulates merges, DecisionPolicy judges"),
    ("E8","SleepCycleExperiment","run_sleep_cycle(): decay weights, prune edges, delete orphans, run gap scanner"),
    ("E9","AttentionModulatorDemo","Compare cluster assignments with/without attention; same stream, different cluster geometry"),
    ("E10","CPILiveFireSimulation","100-experience simulation with GenerativeWorld + PredictiveAgent + ReflectionEngine"),
    ("E11","ProbationaryConceptPipeline","Candidate birth when EXHAUSTED, watch over PROBATION_MIN_TRIALS, confirm if ≥ 0.60 coverage"),
    ("E12","ConceptBirthEngine","Emergent clustering: NPMI thresholding, spectral eigengap, greedy seed-growth, LatentConcept spawning"),
    ("E13","ReplayEngineSandbox","Clone substrate, apply proposed merge, measure H/S/A, produce DecisionScore"),
]

STAGES = [
    ("S1","PreC1_DirichletMarkov","Flat variable-order Markov model with symmetric Dirichlet prior (pre-C6 substrate)"),
    ("S2","C1_PredictiveLoop","Continuous predict/collide/adapt loop; engine always holds a standing prediction"),
    ("S3","C2_ExplainPrediction","Rationale generation per prediction; localized world model with rule + exceptions"),
    ("S4","C3_HypothesisGeneration","Causal hypothesis triggered when surprise exceeds threshold"),
    ("S5","C5_CuriosityInquiry","Curiosity engine selects highest-EIG question about concept-trait relationships"),
    ("S6","C6_StableBelief","Multidimensional beliefs with stability, quarantine, fracture; replaces flat substrate"),
    ("S7","C7_SensoriumProcessing","Continuous vector-based processing; prediction core + latent cause discovery"),
    ("S8","C8_MergeGovernance","Replay sandbox simulates memory restructurings; DecisionPolicy accepts/rejects"),
    ("S9","Stage0_Probation","Probationary concept as temporary overlay (not committed); watched over deviation trials"),
    ("S10","SleepCycle","Synaptic decay, edge pruning, orphan deletion, gap scanning"),
    ("S11","LivingGraphPhase","Hebbian coactivation → edge creation → asymptotic weight reinforcement"),
    ("S12","ResonancePhase","Multi-hop semantic wave propagation through Bayesian living graph"),
    ("S13","DreamState","Subconscious metaphorical linking of charged but unlinked concepts via FAISS"),
    ("S14","MetacognitivePenalty","Bayesian penalty on edges that led to rejected/hallucinated responses"),
    ("S15","OntologyIntegration","Birth concepts passed into World Model Registry and soul graph for reasoning"),
]

RISKS = [
    ("R1","FEPProxyNotFEP","E=λH+μS+νA is NOT variational free energy; no generative likelihood, no posterior approximation; coefficients hand-tuned"),
    ("R2","NoHierarchicalPredCoding","Single-level flat Markov; no hierarchy, canonical microcircuit, or multi-level precision weighting"),
    ("R3","CatastrophicForgettingRisk","C6 Inertia Law + quarantine may not generalize; fracture mechanic invented, not biologically validated"),
    ("R4","OverclaimedNeuroscience","PI review: zero contact with neuroscience; no STDP, neuromodulation, LTP/LTD mechanisms"),
    ("R5","NothingDevelops","Scaffolding is hand-authored; nothing develops — PI review core finding"),
    ("R6","NoSTDP","Rate-based Hebbian only; no spike-timing dependence, no temporal asymmetry"),
    ("R7","NoDopamineNeuromodulation","No reward prediction error, no ACh/NE/5-HT/DA modulation, no three-factor learning"),
    ("R8","NoHomeostasis","No synaptic scaling, firing rate homeostasis, E/I balance, or BCM sliding threshold"),
    ("R9","ActiveInferenceOverclaim","Only learning-rate modulation implemented; no policy selection, no expected free energy"),
    ("R10","ThermodynamicMetaphor","Thermodynamic state has no connection to statistical mechanics; could mislead"),
    ("R11","CuriosityLimited","Only structural hypothesis testing; no learning-progress or competence-based exploration"),
    ("R12","NoSensorimotorGrounding","Piagetian sensorimotor bootstrapping absent; static embeddings lack sensory grounding"),
    ("R13","ConstitutionConflicts","Science (methodology), Uncertainty (honest reporting), Epistemology (traceable sources) at risk"),
    ("R14","ScaffoldingNotEmergent","Fixed curriculum with pre-answered questions; no autonomous learning progress"),
    ("R15","MetaphoricalLanguageMisleads","Resonance, thermodynamic state, sleep cycle are metaphors, not literal neuroscience"),
    ("R16","EpisodicMemoryNotNeural","Episodic memory is a causal event log / SQLite store, not auto-associative network"),
    ("R17","WorkingMemorySimplistic","Simple FIFO key-value store; no central executive, phonological loop, visuospatial sketchpad"),
]

METRICS = [
    ("M1","CPI_Composite","Cognitive Progress Index = weighted blend of accuracy (0.4), decay (0.3), compression (0.2), resolution (0.1); ∈ [0,1]"),
    ("M2","TransitionEntropy_H","Conditional Shannon entropy H(next|current) of cluster-transition Markov chain (bits)"),
    ("M3","SpatialSurprise_S","Euclidean distance between predicted centroid and observed vector"),
    ("M4","ActiveLoad_A","Cluster count + anomaly spatial volume"),
    ("M5","CognitiveEnergy_E","λH + μS + νA; headline free-energy proxy; threshold at 18.0 for exhaustion"),
    ("M6","PredictionAccuracy","EWMA of 1−normalized(surprise); ∈ [0,1]; rising = converging on world"),
    ("M7","SurpriseDecayRate","EWMA of recovery fraction after novelty spike; 1 = instant recovery"),
    ...
    ("M11","ProbationCoverage","hits / trials for probationary concept; must be ≥ 0.60 to confirm"),
    ...
    ("M14","EdgeConfidence","Bayesian state: ACTIVE / CONTESTED / INFERRED / UNCERTAIN / VERIFIED"),
    ...
    ("M17","BottleneckMeanAccuracy","Per-concept mean accuracy; chronic if consolidated but < 0.55"),
    ("M18","PromisedBitsDelta","H_before − H_after at concept birth; predictive uncertainty reduction"),
    ("M19","SurpriseSlope","OLS slope of recent average_surprise; |slope|<ε + level>threshold = plateau"),
]

KILL_CRITERIA = [
    ("K1","CPIDeclining","CPI consistently drops across runs — learning stagnates or reverses"),
    ("K2","CatastrophicForgettingPersists","C6 Inertia Law + quarantine fails to protect mature rules in long-running regimes"),
    ("K3","ConstitutionViolation","Science/Uncertainty/Epistemology constitutions breached"),
    ("K4","NothingDevelopsPersists","Scaffolding remains entirely hand-authored; PI review verdict never addressed"),
    ("K5","SurprisePlateauUnresolved","Average surprise plateaus above stuck_threshold (1.5 bits) with bottleneck < 0.55"),
    ("K6","ExhaustionSustained","Cognitive energy E ≥ 18.0 sustained; fragmentation signal without discovery"),
    ("K7","FractureSupersaturation","fracture_ratio ≥ 1.0 sustained across multiple contexts; rules cannot stabilize"),
    ("K8","NoGenuineLearning","CPI near 0.5 with flat trajectory while surprise stays high"),
    ("K9","FEPMisrepresentation","FEP proxy presented as Friston free energy without qualification fails Science constitution"),
    ("K10","ProbationNeverConfirms","All probationary candidates rejected; hypothesis testing pipeline fails to find valid structures"),
]
...
EDGES_CONTRADICTS = [
    ("L5","L7","C6 replaces precision-weighted learning with Inertia Law; attention no longer modulates learning rate"),
    ...
    ("L7","L5","Inertia (inverse-stability scaling) directly contradicts precision-weighting (surprise-amplified learning)"),
    ...
    ("L19","K8","CPI could show improvement while system not genuinely learning (gaming the metric)"),
...
EDGES_DERIVES_FROM = [
    ...
    ("L4","L3","Shannon surprise of symbol derives from Dirichlet-smoothed predictive distribution"),
    ...
    ("L14","L4","NPMI = log(p(x,y)/p(x)p(y)) / −log p(x,y) derives from pointwise mutual information"),
...
TYPE_MAP = {}
for nodes, typename in [(LAWS,"Laws"),(ASSUMPTIONS,"Assumptions"),(VARIABLES,"Variables"),
                        (EXPERIMENTS,"Experiments"),(STAGES,"Stages"),(RISKS,"Risks"),
                        (METRICS,"Metrics"),(KILL_CRITERIA,"KillCriteria")]:
...
def make_graphml():
    ...
    ET.SubElement(root, "key", id="description", for_="node", attrname="description", attrtype="string")
    ET.SubElement(root, "key", id="edgetype", for_="edge", attrname="edgetype", attrtype="string")
    ET.SubElement(root, "key", id="edgedesc", for_="edge", attrname="description", attrtype="string")
...
def make_mermaid():
    ...
    lines.append("    classDef kill fill:#b71c1c,color:#fff,stroke:#f44336,stroke-width:3px,stroke-dasharray: 5 5")
    lines.append("")
