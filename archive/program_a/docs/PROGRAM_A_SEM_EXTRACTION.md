...
**Object identity:** `SEM-EXTRACTION`, revision `sem-extraction-2026-07-16-r4`
...
dischargeable); `PROGRAM_A_SEM_SUPPORT.md` revision `sem-support-2026-07-16-r2`
...
**Date:** 2026-07-16
**Revision note (r3 → r4):** Sibling revision coupling update. Every import reference that binds to `sem-support-2026-07-15-r1` is updated to the live canonical revision `sem-support-2026-07-16-r2`. The imported semantic identity is unchanged, and the extension of `extract` (and of every exported object) is identical to Revision 3 on every element of `U`. This extension preservation is guaranteed because `SEM_SUPPORT` r2 explicitly states and proves complete extension preservation over its r1 counterpart, ensuring that all imported definitions—including `NORM-1`, `TOK-1`, `ENT-1`, `UVER`, and the `supports` predicate—retain identical mathematical extensions. No other change is made.
...
imports of §24 bind to `sem-support-2026-07-16-r2` by identity; if that
...
| Field | Value |
|---|---|
| Identity | `SEM-EXTRACTION`, revision `sem-extraction-2026-07-16-r4` (supersedes `sem-extraction-2026-07-16-r3` — sibling revision coupling update; r3 superseded `sem-extraction-2026-07-16-r2` — minimality re-audit, extension-preserving removal of inert clauses, header revision note; r2 superseded `sem-extraction-2026-07-15-r1`, rejected at S3 on RC-9) |
...
| References | L0/L1 only, acyclic (§24; J-6): the frozen type contract, the fixed T1/addendum context, and `SEM-SUPPORT` r2 exports (L1→L1, one direction — `SEM-SUPPORT` §24 imports nothing from this object). No reference to any L2/L3/L4/L5 object as authority. |
...
`D` consumes, **by reference to `PROGRAM_A_SEM_SUPPORT.md` r2 §6.1–§6.2/§9
...
`PROGRAM_A_SEM_SUPPORT.md` r2 §6.1–§6.2/§9, verbatim, with no local
...
> contract, the pinned UVER data, and the imported `SEM-SUPPORT` r2 exports)
...
1. Two readers, independent of this author and of each other, receive only:
   ...
   contract, the imported-export sections of `SEM-SUPPORT` r2 (its §6.1,
...
| X-9 | SQ-EXT-9 — entity notion; single-sourcing | §6.1: imported `ENT-1`/`TOK-1`/`NORM-1`/`UVER` from `SEM-SUPPORT` r2 by reference — one owner, this object an importer (J-1 registry, §24). |
...
`SEM-SUPPORT` r2 §17.2; CE-12 is owed by `SEM-MATERIALITY`; CE-9 by
`SEM-TEMPLATE`.
...
§A0–§A7, the frozen type contract, ONT-1, and the imported `SEM-SUPPORT` r2
...
| AC-8 | Ontology well-formedness and role/no-proxy discipline intact across S0–S4 (incl. the SAM-R2 timestamp precedence recorded in the header Status, and the §24 revision coupling to `sem-support-2026-07-16-r2` still valid) | manifests + stage records |
...
- RC-11: invalidation of the §24 revision coupling — rejection or
  supersession of `sem-support-2026-07-16-r2` — which voids the imports'
...
— `SEM-SUPPORT` r2 §24 declares **no** import from `SEM-EXTRACTION`, so the
...
| `supports`, `E(c)`, `V(e)` | imported from `SEM-SUPPORT` r2 (§6.1) |
...
`SEM-EXTRACTION` (rev `sem-extraction-2026-07-16-r4`); clauses C1–C10
